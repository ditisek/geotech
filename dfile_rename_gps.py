def dfile_rename_gps(file_extension="*.d"):
    """
    ...
    file_path = get_file(file_extension)
    new_files = []
    path = file_path.rsplit("\\", 1)[0]
    files = listdir(path)
    files = [i for i in files if i.endswith(file_extension.replace("*", ""))]
    ...
    output_file_name = files[0].rsplit('\\', 1)[0] + "\\file_list.txt"
